// Processes scheduled_reminders rows that are due and should auto-send (email).
...
Deno.serve(async (req) => {
  ...
  try {
    // Fetch due, scheduled, auto-send email reminders (limit batch to be safe)
    const { data: due, error } = await supabase
      .from("scheduled_reminders")
      .select(`
        id, business_id, patient_id, appointment_id, scheduled_for, message, channel, status, auto_send,
        patients!fk_patient ( email, full_name )
      `)
      .eq("status", "scheduled")
      .eq("channel", "email")
      .eq("auto_send", true)
    ...
    for (const r of due || []) {
      const patient = (r as any).patients as { email: string | null; full_name: string | null } | null;
