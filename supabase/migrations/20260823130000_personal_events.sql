-- Eventos personales del profesional ("Gimnasio", "Pediatra", etc.).
-- Viven en la agenda junto a las citas y BLOQUEAN la reserva online:
-- get_available_starts los resta igual que a las citas. Privados del
-- consultorio: los pacientes jamás los ven (sin política para ellos).
...
CREATE TABLE IF NOT EXISTS public.personal_events (
  ...
  -- 'none' = una sola vez; 'weekly' = se repite cada semana el mismo día/hora
  recurrence text NOT NULL DEFAULT 'none',
  recurrence_until date,
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT personal_events_valid_range CHECK (end_at > start_at),
  CONSTRAINT personal_events_recurrence_valid CHECK (recurrence IN ('none', 'weekly'))
);

CREATE INDEX IF NOT EXISTS idx_personal_events_business_start
  ON public.personal_events(business_id, start_at);
CREATE INDEX IF NOT EXISTS idx_personal_events_professional
  ON public.personal_events(professional_user_id);

ALTER TABLE public.personal_events ENABLE ROW LEVEL SECURITY;

DROP TRIGGER IF EXISTS trg_personal_events_updated_at ON public.personal_events;
CREATE TRIGGER trg_personal_events_updated_at
  BEFORE UPDATE ON public.personal_events
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

DROP POLICY IF EXISTS "personal_events_business_members_all" ON public.personal_events;
CREATE POLICY "personal_events_business_members_all"
...
DROP POLICY IF EXISTS "personal_events_super_admin_all" ON public.personal_events;
CREATE POLICY "personal_events_super_admin_all"
  ON public.personal_events
  FOR ALL
...
-- ============================================================
-- get_available_starts: además de las citas, los eventos
-- personales también ocupan el horario (incluida la repetición
-- semanal). Mismo patrón indexable que el chequeo de citas.
-- ============================================================
  ...
  WHILE v_date <= v_end_date LOOP
    ...
    LOOP
      ...
      LOOP
        ...
        -- ¿Algún evento personal ocupa [v_start, v_slot_end)?
        -- 'none': comparación indexable por rango. 'weekly': mismo día de
        -- semana y solapamiento de horas, desde su inicio hasta recurrence_until.
        IF v_busy_end IS NULL THEN
          ...
          WHERE e.business_id = p_business_id
            ...
            AND (
              ...
              OR (
                e.recurrence = 'weekly'
                AND EXTRACT(DOW FROM (e.start_at AT TIME ZONE 'America/Montevideo'))::integer = v_dow
